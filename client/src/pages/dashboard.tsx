...
export default function Dashboard() {
  ...
  const lowStockCount = reels?.filter(r => r.currentStock < 100).length || 0;
  const totalStock = reels?.reduce((acc, curr) => acc + curr.currentStock, 0) || 0;
        ...
        <CardHeader className="border-b bg-slate-50/50">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <CardDescription>Manage your reel specifications and stock levels.</CardDescription>
            </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {isLoading ? (
                <tr>
                  <td colSpan={6} className="h-32 text-center text-muted-foreground">Loading inventory data...</td>
                </tr>
              ) : filteredReels?.length === 0 ? (
                <tr>
                  <td colSpan={6} className="h-32 text-center text-muted-foreground">No reels found matching your search.</td>
                </tr>
              ) : (
                filteredReels?.map((reel) => (
                  <tr key={reel.id} className="group hover:bg-slate-50 transition-colors">
                    <td className="table-cell-dense pl-6 font-medium text-slate-900">{reel.code}</td>
                    ...
                    <td className={`table-cell-dense text-right font-mono font-medium ${reel.currentStock < 100 ? 'text-red-600' : 'text-slate-700'}`}>
                      {reel.currentStock.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                    </td>
